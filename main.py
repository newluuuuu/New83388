...
async def send_error_to_admin(error_message: str, function_name: str):
    """Send database errors to admin instead of user"""
    try:
        from telegram import Bot
        bot = Bot(token=f"{BOT_TOKEN}") 
        
        admin_message = f"🚨 *Database Error Alert*\n\n"
        admin_message += f"📍 *Function:* `{function_name}`\n"
        admin_message += f"🔴 *Error:* `{error_message}`\n"
...
async def get_or_create_client(user_id: str, api_id: str, api_hash: str) -> TelegramClient:
    ...
    if user_id in active_clients:
        ...
        if client.is_connected():
            ...
            # Client exists but not connected, reconnect
            await client.connect()
            return client
...
async def disconnect_client(user_id: str):
    """Safely disconnect and remove client"""
    if user_id in active_clients:
        try:
            client = active_clients[user_id]
            if client.is_connected():
...
async def off(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if user_data.get("forwarding_on", False):
            ...
            if scheduler.running:
                for job in scheduler.get_jobs():
                    if len(job.args) > 2 and job.args[2] == user_id:
            ...
            if not save_user_data(data):
                await send_error_to_admin(f"Failed to save off state for user {user_id}", "off_function")

            response_text = "✅ *Message Forwarding Status*\n\n❌ *Forwarding has been disabled*\n└ _Your automated message forwarding service is now turned off_"
            
            # Stop scheduler if no jobs remaining
        ...
        await send_error_to_admin(f"Error disabling forwarding for user {user_id}: {str(e)}", "off_function")
        error_text = "*An error occurred while disabling forwarding. The admin has been notified.*"
        if update.callback_query:
...
async def offf(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: str, reason: str = "") -> None:
    try:
        ...
        if user_data.get("forwarding_on", False):
            ...
            data["users"][user_id] = user_data
            if not save_user_data(data):
                await send_error_to_admin(f"Failed to save offf state for user {user_id}", "offf_function")
        ...
        await send_error_to_admin(f"Error in offf function for user {user_id}: {str(e)}", "offf_function")
...
async def forward_messages(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: str) -> None:
    try:
        # Check if client is still active for this user
        if user_id not in client_states or not client_states.get(user_id, False):
            await offf(update, context, user_id, reason="Client connection lost")
            return

        data = load_user_data()
        user_data = data["users"].get(user_id, {})
        
        if not user_data.get('forwarding_on', False):
            await offf(update, context, user_id, reason="Forwarding disabled")
            return

        # Use existing client instead of creating new one
        if user_id not in active_clients:
            await offf(update, context, user_id, reason="No active client found")
            return
        ...
        if not await client.is_user_authorized():
            await disconnect_client(user_id)
            await offf(update, context, user_id, reason="Session expired. Please log in again")
            return
        ...
        # Forward messages logic (simplified for brevity)
        current_post = post_message[post_index]
        
        if message_target == 'scraped':
            # Handle scraped users forwarding
                ...
                try:
                    ...
                    if current_post.startswith("https://t.me/"):
                        ...
                        if from_peer and message_id:
                            ...
                            group_name = getattr(target_group, 'title', getattr(target_group, 'username', str(to_peer)))
                            print(f"Message forwarded to {group_name}")
                    else:
                        target_group = await client.get_entity(to_peer)
                    ...
                    error_msg = f"Error forwarding to {group_link}: {str(e)}"
                    await send_error_to_admin(error_msg, f"forward_messages_user_{user_id}")
        ...
        await send_error_to_admin(f"Error in forward_messages for user {user_id}: {str(e)}", "forward_messages")
        await offf(update, context, user_id, reason="An error occurred during forwarding")
...
async def forward_saved(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: str) -> None:
    try:
        ...
        # Forward saved messages logic (similar to forward_messages but for saved messages)
        if message_target == 'scraped':
            for user_to_message in destinations:
                try:
            ...
            for i, group_link in enumerate(destinations):
                try:
                    ...
                    if group_delay > 0 and i < len(destinations) - 1:
                        await asyncio.sleep(group_delay)

                except Exception as e:
                    await track_forward(user_id, False, group_link)
                    failed_count += 1
                    await send_error_to_admin(f"Error forwarding saved message to {group_link}: {str(e)}", f"forward_saved_user_{user_id}")

        await asyncio.sleep(interval)

    except asyncio.CancelledError:
        print(f"Saved message forwarding for user {user_id} was canceled.")
        return
    except Exception as e:
        await send_error_to_admin(f"Error in forward_saved for user {user_id}: {str(e)}", "forward_saved")
        await offf(update, context, user_id, reason="An error occurred during saved message forwarding")
...
async def on(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_data.get("forwarding_on", False):
        await (message.edit_text if is_callback else message.reply_text)("*Forwarding is already active ✅*", parse_mode="Markdown")
        return
    ...
    # Step 2: Create/get client and set client state
    try:
        client = await get_or_create_client(user_id, user_data["api_id"], user_data["api_hash"])
        
        # Step 3: Set client state as ON for this user
        client_states[user_id] = True
        
        # Update forwarding status
        data["users"][user_id]["forwarding_on"] = True
        if not save_user_data(data):
            await send_error_to_admin(f"Failed to save forwarding state for user {user_id}", "on_function")
        ...
        job_exists = any(job.args[2] == user_id for job in scheduler.get_jobs())
        if not job_exists:
        ...
        await send_error_to_admin(f"Error enabling forwarding for user {user_id}: {str(e)}", "on_function")
        await (message.edit_text if is_callback else message.reply_text)(
            "*An error occurred while enabling forwarding. The admin has been notified.*",
            parse_mode="Markdown"
        )
